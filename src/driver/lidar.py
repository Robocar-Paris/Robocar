"""
Docstring for driver.lidar

The Lidar LD19 is a 360-degree 2D laser from Ldrobot
- Range: 0.02m to 12m
- Angular Resolution: 1 degree
- Scan Frequency: 5-15Hz (configurable)
- interface: UART

Protocol:
- Each packet: 47 bytes
- Header: 2 bytes (0x54) start bytes
- Contains 12 measurement points per packet
- CRCB checksum at the end of each packet
...
@dataclass
class LidarPoint:
    """Single point measurement from Lidar."""
    angle: float
    distance: float
    intensity: int
    valid: bool
...
class LidarDriver:
    ...
    def start(self) -> bool:
        ...
        try:
            self._serial = serial.Serial(port=self.port, baudrate=self.baudrate, timeout=1.0)
            self._running = True
            self._thread = threading.Thread(target=self._read_loop, daemon=True)
            self._thread.start()
            return True
        except serial.SerialException as e:
            print(f"Error starting Lidar driver: {e}")
            return False
    ...
    def get_scan(self, timeout: float = 1.0) -> Optional[LidarScan]:
        """
        Get the most recent Lidar scan
        
        Args: timeout: Maximum time to wait for a scan (in seconds)
        
        returns: LidarScan or None
        """
        ...
        try:
            for _ in range(10):
                ...
                if scan:
                    ...
                    print(f"Lidar scan: {valid_points} valid points, "
                          f"freq={scan.scan_frequency:.1f}Hz")
                else:
                    print("Lidar No scan received")
        finally:
            lidar.stop()
    else:
